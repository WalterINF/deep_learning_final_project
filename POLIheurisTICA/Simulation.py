...
from esqueleto.estados import Estado4
from esqueleto.modelo_cinematico_trator_trailer import TratorComUmTrailer    
from esqueleto.parametros_trator_trailer import ParametrosTratorTrailer
from esqueleto.data_class_loader.dc_loader_parametros_veiculo import ListaDeVeiculos
from esqueleto.data_class_loader.dc_loader_expansao import ListaDeParametrosExpansao
# from esqueleto.data_class_loader.dc_loader_tolerancias import ListaDeTolerancias


# tolerancias = ListaDeTolerancias("config/lista_tolerancias.json").get("saida_para_objetivo_acoplada")
parametros_geometricos = ListaDeVeiculos("esqueleto\\lista_veiculos.json").get("VeiculoDesacoplado")
parametros_expansao = ListaDeParametrosExpansao("esqueleto\\lista_parametros_expansao.json").get("15graus_06dt")
# --- 1) Instanciação do modelo ---
parametros_do_veiculo = ParametrosTratorTrailer(parametros=parametros_geometricos)
...
class Simulation:
    ...
    def __init__(self, vehicle: "ArticulatedVehicle", map: "Map"):
        ...
        self.modelo_cinematico = TratorComUmTrailer(parametros_do_veiculo, dt=0.6)
    ...
    def move_vehicle(self, velocity: float, alpha: float, dt: float):
        ...
        estado_atual = Estado4(x, y, theta, beta)
        controle = (velocity, alpha)
        # Update state using kinematic model
        novo_estado = self.modelo_cinematico.atualizar_estado(estado_atual, controle)
        
        self.vehicle.update_physical_properties(novo_estado.x, novo_estado.y, velocity, novo_estado.theta, novo_estado.beta, alpha)
